...
//implementar função de criptografar senha em hash
...
export async function hashPassword(password) {
  try {
    if (typeof password !== "string") {
      return {
        ok: false,
        error: "Senha inválida (tipo incorreto)."
      };
    }
...
WHERE email = ? LIMIT 1`,
  ...
  if (!passOk) {
    // Incrementa o contador de tentativas falhadas, com máximo de 255 para não exceder limite do banco const newFails = Math.min((user.failed_attempts || 0) + 1, 255); 
